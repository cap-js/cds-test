...
class AxiosProvider {
  ...
  get axios() {
    ...
    const agent = process.env.CDS_TEST_PROXY
      ? new ProxyAgent({ getProxyForUrl: () => process.env.CDS_TEST_PROXY, rejectUnauthorized: false })
      : new http.Agent({ keepAlive: false}) //> https://github.com/nodejs/node/issues/47130
