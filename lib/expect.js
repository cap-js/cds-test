const DEBUG = /\btest\b/.test(process.env.DEBUG) ? console : undefined
DEBUG?.warn('Using chai.expect surrogate.')
...
class Core {
  ...
  property (p, v) {
    ...
    if (this._not) return this._nested
      ? this.should`not have nested property ${p}`
      : this.should`not have property ${p}`
    return that
  }
...
class AssertionError extends Error {
  constructor (m, caller = Assertion.prototype.should) {
    super(m)
    Error.captureStackTrace(this, caller)
    // clean up stack trace so that actual test file is top-most frame
    if (!DEBUG && this.stack) {
      this.stack = this.stack.split('\n')
        .filter(line => !line.match(/\s+at.*[/\\]lib[/\\]expect\.js:/))
        // clean up TestContext/SuiteContext noise but preserve file location
        .map(line => line.replace(/TestContext\.<anonymous>|SuiteContext\.<anonymous>/gi, 'Test'))
        .join('\n')
    }
  }
