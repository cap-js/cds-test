...
class Chai extends Core {
  ...
  get contains() { return new Proxy (this.includes,{
    ...
    get: (fn,k) => {
      if (k === 'deep') {
        // FIXME: _deep seems to be treated as a boolean at every call site. Why do we assign a function here?
        this._deep = fn
        return (/** @type{any[]} */...args) => fn.call(this, ...args)
      }
      // @ts-expect-error - apparently, fn is either a function, or, I guess, an object
      else return fn[k]
    },
...
class Jest extends Chai {
  ...
  // FIXME: this.deep is treated as a boolean throughout this file (is a function at one point). Why do we assume arrayesque functionality here?
  ...
  toHaveBeenCalledWith (...args) {
    return this.assert (
      fn => fn.mock.calls.some((/** @type {{arguments: any}}*/c) =>
        compare(c.arguments, args, true)),
