...
class Test extends require('./xaxios') {
  ...
  run (folder_or_cmd, ...args) {
    ...
    const self = this
    const isJava = cds.env.profiles.includes('java') && !cds.env.profiles.includes('node')

    // launch cds server...
    const serveTestRuntime = async function () {
    ...
    if (isJava) serveTestRuntime._hookTimeout = 30000
    before (serveTestRuntime)
