...
class Test extends require('./xaxios') {
  ...
  run (folder_or_cmd, ...args) {
    ...
    const _root = cds.root, _exec = cds.exec
    ...
    // gracefully shutdown cds server and reset cds global so the next test file
    // in the same mocha parallel worker starts with a clean state
    after (async ()=> {
      await Promise.all([
