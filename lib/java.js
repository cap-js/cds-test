...
const startJavaAppProcess = async (url, readyPath, p, cds, path, fs) => {
  ...
  return await new Promise((resolve, reject) => {
    ...
    const ping = () => cds.test.axios.get(url + readyPath).catch(() => sleep(500).then(ping));
...
module.exports = async function java(...args) {
  ...
  // Derive a reliable readiness URL from the first OData service in the model
  const svc = Object.values(model.definitions).find(d => d.kind === 'service' && d['@path'])
  const readyPath = svc ? `/odata/v4/${svc['@path']}` : '/'

  const app = await startJavaAppProcess(url, readyPath, p, cds, path, fs);
