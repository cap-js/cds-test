...
const buildDatabaseProxy = async (cds, from) => {
  ...
  // Resolves a service or DB entity name to its dbProxy key, following projection chains (service view → DB entity) recursively.
  // Falls back to the given name when no projection chain can be found (e.g. external or virtual entities).
  const resolveProxyTarget = name => {
    if (isDBEntity(name)) return nameToProxyKey(name)
    const def = linked.definitions[name]
    const fromRef = def?.query?.SELECT?.from?.ref ?? def?.projection?.from?.ref
    if (fromRef?.length) return resolveProxyTarget(fromRef[0])
    return name
  }
  ...
  const buildActiveElements = (elements = {}) => {
    ...
    for (const [name, el] of Object.entries(elements)) {
      ...
      if (el.type === 'cds.Association') {
        ...
        if (el.target) associationElement.target = isDBEntity(el.target) 
          ? nameToProxyKey(el.target)
          : resolveProxyTarget(el.target)
  ...
  const buildDraftElements = (elements = {}) => {
    ...
    for (const [name, el] of Object.entries(elements)) {
      ...
      if (el.type === 'cds.Association') {
        ...
        if (el.target) associationElement.target = draftEntityNames.has(el.target)
          ? nameToProxyKey(el.target + '.drafts')
          : resolveProxyTarget(el.target)
        if (el.keys)        associationElement.keys        = el.keys
        if (el.on)          associationElement.on          = el.on
        if (el.cardinality) associationElement.cardinality = el.cardinality

        draftEntityElements[name] = associationElement
      ...
      draftEntityElements[name] = scalarElement
    }
