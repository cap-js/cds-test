...
// Return the flat local name for the dbProxy entity:
// 2-segment: bookshop.Authors → Authors
// 3-segment: bookshop.Books.texts → BooksTexts, CatalogService.Books.drafts → BooksDrafts
function localProxyName(name) {
  // TODO: I don't think we can rely on the number of segments
  // TODO: > The number of db entities should be limted and known from loading the model ... can't we just use that knowledge to compile a deterministic mapping of identifiers? 
  const segments = name.split('.')
  if (segments.length <= 1) return name
  if (segments.length === 2) return segments[1]
  const child = segments[segments.length - 1]
  const parent = segments[segments.length - 2]
  return parent + child[0].toUpperCase() + child.slice(1)
}
...
const injectTestRuntimeDatabaseProxy = async (cds, options) => {
  ...
  for (const [name, def] of Object.entries(model.definitions)) {
    ...
    const proxyKey = 'dbProxy.' + localProxyName(name)
  ...
  for (const [name, def] of Object.entries(linked.definitions)) {
    ...
    const proxyKey = 'dbProxy.' + localProxyName(draftName)
