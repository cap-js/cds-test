...
/**
 * Produces the `elements` block for a proxy entity declaration in CSN format.
 *
 * The HCQL database proxy is a synthetic CDS service that exposes every physical database
 * table as a queryable entity, routing CDS ql queries transparently to a running Java
 * application via HTTP. Each proxy entity needs only a minimal field description — the CDS
 * runtime does not need service-layer artefacts that have no physical column.
 *
 * This function distils a real entity's elements to that minimum:
...
/**
 * Orchestrates the full database proxy setup for a Java test run.
 *
 * Builds the `dbProxy` HCQL service definition via {@link buildProxyEntityElements} and
 * {@link buildProxyDraftElements}, writes it to `db-proxy.{cds,json}`, reloads `cds.model`,
 * and wires the `db` service to the HCQL proxy adapter (`java-hcql.js`).
 *
 * Two passes over all entity definitions are required: Pass 1 assigns every entity a unique
 * proxy name; Pass 2 builds the element descriptors. A single pass is not possible because
 * association targets must resolve to proxy names that may not yet have been assigned.
 *
...
const injectTestRuntimeDatabaseProxy = async (cds, options) => {
  ...
  // For each draft-enabled entity, register proxy tables for it and its entire composition tree.
