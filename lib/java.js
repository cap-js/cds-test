...
const buildDatabaseProxy = async (cds, from) => {
  ...
  // Load the app model. Old proxy definitions in srv/ are safely ignored by isDBEntity
  // (@cds.persistence.exists: true), so unlinking before load is not necessary — and
  // doing so would create a race when multiple test workers run concurrently.
  ...
  // Builds the elements block for an active-entity proxy. Compositions and associations are
  // remapped to proxy service names; unmanaged associations and FK aliases are dropped.
  const buildActiveElements = (elements = {}) => {
    const result = {}
    for (const [name, el] of Object.entries(elements)) {
      if (el['@odata.foreignKey4']) continue
      if (el.type === 'cds.Association' && !el.keys) continue  // unmanaged: no FK columns
      ...
      if (el.type === 'cds.Composition') {
        ...
        result[name] = {
          ...
          ...(el.cardinality && { cardinality: el.cardinality }),
          ...(el.on && { on: el.on })
        }
        continue
      }
      ...
      if (el.type === 'cds.Association') {
        result[name] = {
          ...
          ...(el.target && { target: isDBEntity(el.target) ? nameToProxyKey(el.target) : el.target }),
          ...(el.keys && { keys: el.keys })
        }
        continue
      }
  ...
  // Builds the elements block for a draft-entity proxy. Draft tables are physically flat:
  // associations expand to FK columns, compositions point to *.drafts proxy targets with
  // remapped on-conditions, and four CAP draft metadata columns are appended.
    ...
    for (const [name, el] of Object.entries(elements)) {
      ...
      if (el.type === 'cds.Composition') {
        ...
        result[name] = {
          ...
          ...(el.cardinality && { cardinality: el.cardinality }),
          ...(el.on && { on: remapDraftJoinConditions(el.on, linked.definitions[el.target]?.elements ?? {}) })
        }
        continue
      }
      ...
      if (el.type === 'cds.Association') {
        ...
        for (const { ref: [keyName] } of (el.keys ?? [])) {
          ...
          result[`${name}_${keyName}`] = { type: targetKeyEl?.type ?? 'cds.UUID' }
        }
        continue
      }
...
function remapDraftJoinConditions(conditions, targetElements) {
  ...
  return conditions.map(condition => {
    ...
    const [target, on] = condition.ref
    if (on) {
      const assoc = targetElements[on]
      if (assoc?.type === 'cds.Association' && assoc.keys?.length) {
        keyName = assoc.keys[0].ref[0]
        return { ref: [target, `${on}_${keyName}`] }
      }
    }
    if (target === '$self' && !on && keyName) return { ref: ['$self', keyName] }
    return condition
  })
}
