...
module.exports = async function java(...args) {
  ...
  const app = await new Promise((resolve, reject) => {
    ...
    const startupTimer = setTimeout(() => {
      javaAppProcess.kill()
      settle(reject, new Error('Java application did not respond within 90s — killed. Check the output above for details.'))
    }, 90_000)

    javaAppProcess.on('error', err => settle(reject, err))
    javaAppProcess.on('exit', (code, signal) => {
      const reason = signal ? `killed by signal ${signal}` : `exited with code ${code}`
      settle(reject, new Error(`Application failed to start — process ${reason}. Check the application output above for details.`))
    })
