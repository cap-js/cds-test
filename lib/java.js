 ...
 * service that exposes every physical database table as a proxy entity, with the goal of 
 * enabling the same query patterns to route transparently to the Java backend via HCQL.
 *
...
const buildDatabaseProxy = async (cds, from) => {
  ...
  const resolvedTypeOf = el => {
    let p = el; while (p && p !== Object.prototype) {
  ...
  // DRAFT.DraftAdministrativeData is a CAP Java framework entity — not in the CDS app model.
  // Add one proxy entry for it whenever the model has draft-enabled entities.
  if (draftEntityNames.size > 0) {
    hcqlDatabaseProxy.definitions[proxyNameOf('DRAFT.DraftAdministrativeData')] =
      proxyEntityDef('DRAFT.DraftAdministrativeData', {
...
const acquireBuildLock = async (lockFile, cds) => {
  ...
  while (true) {
    ...
    if (lock) { await lock.close(); return }
    if (Date.now() > deadline) cds.error(
      `Timed out waiting for Maven build lock after ${MVN_LOCK_TIMEOUT}s`
    )
...
module.exports = async function java(...args) {
  ...
  const appName = await fs.readFile(pomFile, 'utf8')
    .then(xml => xml.replace(/<parent>[\s\S]*?<\/parent>/, '')
    .match(/<artifactId>([^<]+)<\/artifactId>/)?.[1] ?? 'app')
    .catch(() => 'app')

  const jarFile = path.resolve(cds.root, cds.env.folders.srv, `target/${appName}-exec.jar`)

  // Lock serializes concurrent workers — the second worker finds proxy and jar already built.
  ...
  // Reserve a free port immediately before starting Java — avoids TOCTOU race during build.
  const p = await port()
  ...
  // .drafts is linked on service entities but not on their underlying db entities — propagate it.
  for (const [draftName, proxyKey] of Object.entries(proxyMap)) {
    ...
    // how drafts entities are usually attached to actives
  ...
  const app = await new Promise((resolve, reject) => {
    ...
    let settled = false
    const settle = (fn, arg) => { if (!settled) { settled = true; clearTimeout(startupTimer); fn(arg) } }

    // Abort if Java doesn't start serving
    ...
    const ping = () => {
      if (settled) return
      return axios.get(url).then(() => settle(resolve, javaAppProcess)).catch(() => sleep(500).then(ping))
    }

    ping() // ... and start waiting for a response from Java
  })

  const java = { crashed: null, pid: app.pid }

  app.on('exit', (code, signal) => { if (!shutdownIntended) java.crashed = { code, signal } })

  cds.shutdown = () => {
    shutdownIntended = true
    // Detach the HCQL proxy so the next test file reconnects to a new Java process.
    delete cds.services.db; delete cds.db
    app.removeAllListeners('exit')
    if (SKIP_AWAIT_SHUTDOWN) { app.unref(); app.kill(); return }
  ...
  cds.env.requires.db = { impl: require.resolve('./java-hcql.js'), axios, proxyMap, java }

  await cds.connect.to('db')

  // Inject after connect to skip odata compilation
