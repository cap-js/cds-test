...
const proxyNameOf = entityName => 'dbProxy.' + entityName.replace(/\./g, '_')
  ...
  const buildActiveElements = (elements = {}) => {
    ...
    for (const [name, el] of Object.entries(elements)) {
      if (el.type === 'cds.Composition') {
        if (!isDatabaseEntity(el.target)) continue

        const compositionElement = {
          type: 'cds.Composition',
          target: proxyNameOf(el.target)
        }
      ...
      if (el.type === 'cds.Association') {
        ...
        if (el.target) associationElement.target = determineProxyTarget(el.target)
  ...
  const buildDraftElements = (elements = {}) => {
    ...
    for (const [name, el] of Object.entries(elements)) {
      ...
      if (el.type === 'cds.Composition') {
        ...
        const compositionElement = {
          type: 'cds.Composition',
          target: proxyNameOf(el.target + '.drafts'),
        }
      ...
      if (el.type === 'cds.Association') {
        ...
        if (el.target) associationElement.target = determineProxyTarget(el.target, true)
        if (el.keys)        associationElement.keys        = el.keys
        if (el.on)          associationElement.on          = el.on
        if (el.cardinality) associationElement.cardinality = el.cardinality
