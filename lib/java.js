...
const injectTestRuntimeDatabaseProxy = async (cds, options) => {
  ...
  // by cds.load('*') as a duplicate of the per-invocation tmpDir proxy below.
  ...
  // Isolated temp dir — each invocation gets its own proxy files (no write-race).
  const tmpDir = await fs.mkdtemp(
    path.join(require('os').tmpdir(), 'cds-java-proxy-'))
  ...
  await Promise.all([
    fs.writeFile(path.join(tmpDir, 'db-proxy.cds'), `using from './db-proxy.json';`),
    fs.writeFile(path.join(tmpDir, 'db-proxy.json'), JSON.stringify(hcqlDatabaseProxy)),
  ])

  // Reload cds.model with the proxy, then activate CAP draft handling.
  cds.model = await cds.load([...from, path.join(tmpDir, 'db-proxy.cds')])
...
module.exports = async function java(...args) {
  ...
  const { proxyMap, tmpDir } = await injectTestRuntimeDatabaseProxy(cds, options)
...
module.exports.generateProxy = async function(appDir) {
  ...
  const { tmpDir } = await injectTestRuntimeDatabaseProxy(cds, {})
  const srvDir = path.resolve(cds.root, cds.env.folders.srv)
  await Promise.all([
    fs.copyFile(path.join(tmpDir, 'db-proxy.cds'), path.join(srvDir, 'db-proxy.cds')),
    fs.copyFile(path.join(tmpDir, 'db-proxy.json'), path.join(srvDir, 'db-proxy.json')),
  ])
...
module.exports._injectTestRuntimeDatabaseProxy = (cds, options) => {
  ;({ fs: { promises: fs }, path } = cds.utils)
  return injectTestRuntimeDatabaseProxy(cds, options)
}
